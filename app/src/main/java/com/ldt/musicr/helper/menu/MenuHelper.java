...
public class MenuHelper {
    ...
    public static boolean handleMenuClick(@NonNull AppCompatActivity activity, @NonNull Album album, int string_res_option) {
        switch (string_res_option) {
            case R.string.play:
                MusicPlayerRemote.openAndShuffleQueue(album.getSongs(),true);
                return true;
            case R.string.play_next:
                MusicPlayerRemote.playNext(album.getSongs());
                return true;
            case R.string.play_preview:
                if(activity instanceof MusicServiceActivity) {
                    SongPreviewController preview = ((AppActivity) activity).getSongPreviewController();
                    if (preview != null) {
                        if (preview.isPlayingPreview())
                            ...
                            ArrayList<Song> list = new ArrayList<>(album.getSongs());
                            Collections.shuffle(list);
                            preview.previewSongs(list);
                        }
                    }
                ...
                return true;
            case R.string.add_to_queue:
                MusicPlayerRemote.enqueue(album.getSongs());
                return true;
            case R.string.add_to_playlist:
                AddToPlaylistDialog.create(new ArrayList<>(album.getSongs())).show(activity.getSupportFragmentManager(), "ADD_PLAYLIST");
                return true;
        }
        return false;
    }
    ...
    public static boolean handleMenuClick(@NonNull AppCompatActivity activity, @NonNull Object object, int string_res_option) {
        if(object instanceof Song) {
            ...
            return handleMenuClick(activity,(Album)object,string_res_option);
        return false;
    }
}
