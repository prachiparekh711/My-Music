...
public class SongLoader {
    ...
    public static List<Song> searchSongs(Context context, String searchString, int limit) {
        ArrayList<Song> result = getSongs(makeSongCursor(context, "title LIKE ?", new String[]{searchString + "%"}));
        if (result.size() < limit) {
            result.addAll(getSongs(makeSongCursor(context, "title LIKE ?", new String[]{"%_" + searchString + "%"})));
        }
